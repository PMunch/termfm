...
from xresources import XResources
...
type
  LSColors* = object
    ...
    setuid*: Color
    setgid*: Color
    stickyOtherWritable*: Color
    otherWritable*: Color
    sticky*: Color
    executable*: Color
    missing*: Color
    extensions*: Table[string, Color]
...
proc parseColor(colors: XResources, ansi: string): Color =
  var sgr = ansi.split(';').map(parseInt)
  var i = 0
  while i < sgr.len:
    if sgr[i] == 0:
      result = colors.foreground
    elif sgr[i] >= 30 and sgr[i] <= 37:
      result = colors.colors[sgr[i] - 30]
    elif sgr[i] >= 90 and sgr[i] <= 97:
      result = colors.colors[sgr[i] - 90 + 8]
    elif sgr.len > i + 2 and sgr[i] == 38 and sgr[i + 1] == 5:
      let color = extraColors[sgr[i + 2]]
      result = Color(r: color shr 16, g: (color shr 8) and 0xFF, b: color and 0xFF, a: 255)
      i += 2
    elif sgr.len > i + 4 and sgr[i] == 38 and sgr[i + 1] == 2:
      result = Color(r: sgr[i + 2], g: sgr[i + 3], b: sgr[i + 4], a: 255)
      i += 4
    inc i

proc loadColors*(colors: XResources): LSColors =
  result.normal = colors.foreground
  result.file = colors.foreground
  result.directory = colors.foreground
  result.link = colors.foreground
  result.pipe = colors.foreground
  ...
  result.stickyOtherWritable = colors.foreground
  result.otherWritable = colors.foreground
  result.sticky = colors.foreground
  result.executable = colors.foreground
  result.missing = colors.foreground
  for entry in getEnv("LS_COLORS").split ':':
    ...
    let
      ...
      value = pair[1]
    case key:
    of "no": result.normal = parseColor(colors, value)
    of "fi": result.file = parseColor(colors, value)
    of "di": result.directory = parseColor(colors, value)
    of "ln": result.link = parseColor(colors, value)
    of "pi": result.pipe = parseColor(colors, value)
    of "do": result.door = parseColor(colors, value)
    of "bd": result.blockDevice = parseColor(colors, value)
    of "cd": result.charDevice = parseColor(colors, value)
    of "or": result.orphan = parseColor(colors, value)
    of "so": result.socket = parseColor(colors, value)
    of "su": result.setuid = parseColor(colors, value)
    of "sg": result.setgid = parseColor(colors, value)
    of "tw": result.stickyOtherWritable = parseColor(colors, value)
    of "ow": result.otherWritable = parseColor(colors, value)
    of "st": result.sticky = parseColor(colors, value)
    of "ex": result.executable = parseColor(colors, value)
    of "mi": result.missing = parseColor(colors, value)
    else:
      if key.startsWith "*.":
